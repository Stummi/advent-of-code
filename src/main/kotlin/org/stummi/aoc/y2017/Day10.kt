...
object Day10 : AdventOfCode(2017, 10) {
    ...
    override val part1: Any
        get() {
            val knot = (0..255.orSample()).toMutableList()
    ...
    override val part2: Any
        get() {
            val knot = (0..255.orSample()).toMutableList()
            val swaps = input.lines.first().chars().toList() + listOf(17, 31, 73, 47, 23)
            val swapped = doSwaps(knot, swaps, 64)
            return swapped.chunked(16).map { it.reduce(Int::xor) }.map {
